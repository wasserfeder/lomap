...
Edges: {edges}
    ...
    def get_next_states(self, state, input_symbol, output_symbol, weight): # I should use it in the product module too

        #defining the variables
        state = stack.pop() # marks the state we are at the moment (?)

        input_symbol = self.symbols_w_prop(props) # this is the set of props/labels that come w the fsa states (a,b,ab,empty), it indicates current state
        output_symbol = self.symbols_w_prop(props) # this is the set of props/labels that belong to the output state / mext state
        var = 0
        weight = self.weight_function(var)

        next_states = self.g.out_edges_iter(q, data=False) # data=False --> returns a 2-tuple(u,v)
                                                           # q refers to the neighboring state? 

        return next_states # the pair of tuples: label (a,b,ab, empty) / the weight /
        #the output_symbol (symbol that indicates next state)

        # it returns instead of just the input, the output symbol and the weight
        # i think the return/output should be in the form of a pair of tuples
