...
def dijkstra_path(G, source, target, parameter=1, weight='weight', obj_data=False):
    ...
    (length, path, obj1, obj2) = single_source_dijkstra(G, source, target=target,
                                            weight=weight,parameter=parameter, obj_data=obj_data)
    ...
    try:
        ...
        # print("o1_upar:", obj1[target])
        # print("o2_upar:", obj2[target])
        # print("PATH:", path[target])


        return path[target], obj1[target], obj2[target]
        # return path[target]
...
def dijkstra_path_length(G, source, target, parameter=1, weight='weight', obj_data=False):
    ...
    bidirectional_dijkstra()
    """
    length = single_source_dijkstra_path_length(G, source, weight=weight,parameter=parameter, obj_data=obj_data)
    # print("length_target:",length)
    try:
        # print("target_cost:", length[target])
...
def single_source_dijkstra_path(G, source, cutoff=None, weight='weight', parameter=1, obj_data=False):
    ...
    """
    (length, path, o1, o2) = single_source_dijkstra(
        G, source, cutoff=cutoff, weight=weight, parameter=parameter, obj_data=obj_data)
    return path


def single_source_dijkstra_path_length(G, source, cutoff=None,
                                       weight='weight', parameter=1, obj_data=False):
    ...
    return _dijkstra(G, source, get_weight, cutoff=cutoff,parameter=parameter, obj_data=obj_data)


def single_source_dijkstra(G, source, target=None, cutoff=None, weight='weight',parameter=1, obj_data=False):
    ...
    if G.is_multigraph():
        ...
        # weight_value = get_weigh

    else:
        get_weight = lambda u, v, data:  data.get(weight, 1)
        # print("weights: ", get_weight)
        # weight_value = get_weight



    # get_weight = 0.1 * weight_value * parameter

    paths = {source: [source]}  # dictionary of paths
    return _dijkstra(G, source, get_weight, paths=paths, cutoff=cutoff,
                     target=target, parameter=parameter, obj_data=obj_data)
...
def _dijkstra(G, source, get_weight, pred=None, paths=None, cutoff=None,
              target=None, parameter=1, obj_data=False):
    ...
    obj2={}
    while fringe:
        (d,obj_one, obj_two,_, v) = pop(fringe)
        # print("pop:", (d, _, v))
        if v in dist:
    ...
    if paths is not None:
        ...
        if (obj_data == True):
            print("<><><><><><<><><><><>")
            print("obj1_prev:", obj1[target])
            # print("\n\n\n")
            print("obj2_prev:", obj2[target])
            print("<><><><><><><><><><><>")


            return (dist, paths, obj1, obj2)
