        ...
        for ts_next_state in it.chain(ts.g[ts_state], (None,)):
            ...
            for wfse_out in wfse.next_states(wfse_state, ts_next_prop):
                ...
                if next_prop_relax is None:
                    ...
                    print("fsa_next_state:", fsa_next_state)
                else:
    ...
    while stack:
        ...
        for each_edge in wfse.g.out_edges_iter(wfse_current, data=True):
            ...
            for each_edge in edge_data['symbols']:
                ...
                print("check syms: ", wfse_next_state, in_sym,  out_sym)
                print("in out props:", in_prop, out_prop)
                # in_symbol = wfse.prop_from_bitmap(set([in_prop]))
                # out_symbol = wfse.bitmap_of_props(set([out_prop]))
                prop = [(in_prop, out_prop, wfse_weight)]

                if out_prop is None:
                    fsa_next_state = fsa_state
                else:
                    # fsa_next_state = fsa.next_state(fsa_current, out_symbol)
                    fsa_next_state = fsa.next_state(fsa_current, set([out_prop]))
                    print("fsa next state:", fsa_next_state)
                if fsa_next_state is not None:
                    ...
                    elif next_state in product_model.g[current_state]:
                        print("edge to be added here", product_model.g[current_state][next_state]["prop"])
                        # in_prop_prev, out_prop_prev, weight = product_model.g[current_state][next_state]["prop"]
                        # out_prop_prev.append(out_prop)
                        # in_prop_prev.append(in_prop)
                        # print("new_edge_data", out_prop_prev)
                        product_model.g[current_state][next_state]["prop"].append((in_prop, out_prop,wfse_weight))
                        # product_model.g.add_edge(current_state, next_state,
                        #                          weight = weight, prop=)
                        print("============")
...
def product_wfse_fsa(wfse, fsa,  from_current=False, expand_finals=True,
                       get_transition_data=pfsa_default_transition_data):
    automata = [wfse, fsa]
    ...
    while stack:
        ...
        if wfse_current in wfse.final and fsa_current in fsa.final:
            product_model.final.add(current_state)
            product_model.g.add_node(current_state)
        # Iterate over all possible transitions

        for each_edge in wfse.g.out_edges_iter(wfse_current, data=True):
        # for each_prop in wfse.bitmap_of_props(wfse.props):
            for wfse_out in wfse.next_states(wfse_current, each_prop):
                wfse_next_state, next_prop_relax, wfse_weight = wfse_out
                print("wfse next state", wfse_next_state, next_prop_relax, wfse_weight )
                # wfse_syms = wfse.g[wfse_current][wfse_next_state]["symbols"]
                # print("symbols:", wfse_syms)

                # edge_data = wfse.g.get_edge_data(wfse_current, wfse_next_state)

                # print("edge data: ", edge_data)
                # for each_translation in wfse_syms:
                #     print("translation", each_translation)
                #     in_prop, out_prop, wfse_weight = each_translation
                #     # print("check syms: ", wfse_next_state, in_sym,  out_sym)
                #     # in_symbol = wfse.symbol_from_bitmap(in_prop)
                #     # out_symbol = wfse.symbol_from_bitmap(out_prop)
                #     prop = (in_prop, out_prop)
                #     print("wfse translations: ", in_prop, out_prop, wfse_next_state)
                #     if out_prop is None:
                #         fsa_next_state = fsa_state
                #     else:

                fsa_next_state = fsa.next_state(fsa_current, next_prop_relax)
                prop = (next_prop_relax)
                in_prop_bit = wfse.bitmap_of_props(set([each_prop]))
                out_prop_bit = wfse.bitmap_of_props(next_prop_relax)
                print('in,out', in_prop_bit, out_prop_bit)

                in_symbol = set([each_prop])
                out_symbol = next_prop_relax
                if fsa_next_state is not None:
                    next_state = (wfse_next_state,
                                  fsa_next_state)
                    weight = wfse_weight
                    ...
                    if next_state not in product_model.g:
                        ...
                        # Add weighted transition
                        product_model.g.add_edge(current_state, next_state,
                                                 weight=weight, input=out_prop_bit, symbols = (in_symbol, out_symbol))
                        # product_model.g.add_edge(current_state, next_state,
                        #                          weight=weight, prop=prop)
                        # Mark as final if final in fsa
                        if (fsa_next_state in fsa.final
                        ...
                        product_model.g.add_edge(current_state, next_state,
                                                 weight=weight, prop=out_prop_bit, symbols = (in_symbol, out_symbol))
                        # product_model.g.add_edge(current_state, next_state,
                        #                          weight = weight, prop=prop)

                    # else:
                    #     print("state already in product")
                    #     in_data, out_data = product_model.g[current_state][next_state]['symbols']
                    #     out_data.add(out_symbol)
                    #     print('out symbol {}'.format(out_symbol), 'prop', prop,
                    #           'in {}, out {}'.format(in_data, out_data))


    return product_model


def wfse_dfa_product(wfse, fsa):
    ''' This product is well-defined for a DFA'''

    product_props = fsa.props
    ...
    while stack:
        product_current_state = stack.popleft()
        wfse_current, fsa_current = product_current_state
        print("current wfse, current fsa: ", wfse_current, fsa_current)

        if wfse_current in wfse.final and fsa_current in fsa.final:
            product_model.final.add(product_current_state)

        ## Iterate over all outgoing edges of WFSE for each input output symbol pair

        for _, wfse_next, data in wfse.g.out_edges(wfse_current, data=True):
            print("wfse_next:", wfse_next, data)
            for each_sym in data['symbols']:
                print("each pair:", each_sym)

                in_sym_bit, out_sym_bit, weight = each_sym
                ...
                if in_sym_bit == -1:
                    ...
                    in_sym = wfse.symbol_from_bitmap(in_sym_bit)
                    out_sym = wfse.symbol_from_bitmap(out_sym_bit)

                for _, fsa_next, fsa_data in fsa.g.out_edges(fsa_current, data=True):
                    print('next in fsa', fsa_next, 'with data', fsa_data)

                    if out_sym_bit in fsa_data['input']:
                        print('fsa nexts', fsa_next, 'for input', set([out_sym_bit]), 'for current state', fsa_current)

                        next_product_state = (wfse_next, fsa_next)
                        print("next in product: ", next_product_state)

                        if wfse_next in wfse.final and fsa_next in fsa.final:
                            product_model.final.add((wfse_next, fsa_next))

                        if next_product_state not in product_model.g.nodes():
                            product_model.g.add_node(next_product_state)
                            product_model.g.add_edge(product_current_state, next_product_state, input=[(in_sym_bit, out_sym_bit, weight)],
                                                     weight=weight, symbols=[(in_sym, out_sym, weight)])
                            stack.append(next_product_state)


                        elif next_product_state not in product_model.g[product_current_state]:
                            '''node exists; edge doesnt'''
                            product_model.g.add_edge(product_current_state, next_product_state, input=[(in_sym_bit, out_sym_bit, weight)],
                                                     weight=weight, symbols=[(in_sym, out_sym, weight)])


                        else:
                            '''Node already added to product and explored in stack'''
                            if (product_current_state, next_product_state) in product_model.g.edges():

                                ''' edge already added to product; append the inputs and symbols'''
                                print('current product nodes:', product_model.g.nodes())
                                print('current', product_current_state, 'next', next_product_state)
                                product_model.g[product_current_state][next_product_state]['input'].append((in_sym_bit, out_sym_bit, weight))
                                product_model.g[product_current_state][next_product_state]['symbols'].append((in_sym, out_sym, weight))

                                ''''else the edge doent exist; continue'''
    return product_model
