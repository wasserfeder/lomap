...
def draw_grid(ts, edgelabel='control', prop_colors=None, current_node=None):
    ...
    if current_node == 'init':
        current_node = next(iter(list(ts.init.keys())))
...
#     print 'index:', index
    stack = [(current_pa_state, iter(list(pa.g[current_pa_state].keys())),
...
#                     print 'optimum reward', cr, policy
            ...
            try:
                ...
                stack.append((nq, iter(list(pa.g[nq].keys())), ncr))
...
def compute_receding_horizon_policy_dp(pa, current_pa_state,
    ...
    for h in range(horizon):
        ...
        paths = dict()
        for pa_state, pa_path in list(prev_paths.items()):
    ...
    policy = None
    maxr = 0
    for pa_state, pa_path in list(paths.items()):
...
#         print u, d
    ...
    horizon=2
    current_pa_state = next(iter(list(pa.init.keys())))
    policy = None
    while True:
        current_ts_state, _ = current_pa_state
...
#                                          neighborhood_rewards, horizon, policy)
        ...
        for v, r in list(neighborhood_rewards.items()):
